...
const ReduceImgBtnStyle = {
	width: '1000px', // 원하는 너비로 설정
	height: '60px', // 원하는 높이로 설정
	border: '1px solid #585858',  // 테두리 스타일
	background: 'white', // 배경색 지정 (선택 사항)
	color: '#424242', // 텍스트 색상 지정 (선택 사항)
	borderRadius: '2px', // 버튼을 둥글게 만들기 위한 속성
	fontWeight: 'bold',
	fontSize: '19px',
	marginLeft:'90px',
}
...
const shoppingBagBtnStyle = {
	//marginLeft: '90px',
	fontSize: '15px',
	background: '#585858', // 검은색 바탕
	color: '#fff', // 텍스트 색상 (흰색)
	padding: '20px 40px', // 패딩 설정 (상하 10px, 좌우 20px)
	border: 'none', // 테두리 없음
...
const ProductDetail = () => {
	...
	useEffect(() => {
		...
		httpReq.then(response => {
			const splitImgUrl = response.data.ProductImgInfo.ImgURL
			setMainImgPath(splitImgUrl)
	...
	return (
		<div /*style={containerStyle}*/>
			...
			<div className='main-page-dir'/*style={bottomPaneContainerStyle}*/>
				...
				{/*왼쪽*/}
				<div className='detail-left-style' /*style={bottomLeftPaneStyle}*/>
					
				</div>
				
				{/*중앙*/}
				<div className='bottom-center-pane' /*style={bottomCenterPaneStyle}*/>
					<div className='product-detail-page' /*style={{ display: 'flex', flexDirection: 'row' }}*/>
						<img className='product-main-size' src={mainImgPath} alt={"불러오기 실패2"} />
						<div className='product-description-style' /*style={{textAlign: 'left', marginLeft: '90px' }}*/>
							...
							<HeartControl
								 customStyle={{width: '100px', left: leftPos, top: topPos}}
								 productId={productIdStr}
								 propIsLike={isLike}/>
							<div style={{ display: 'flex', flexDirection: 'column', marginLeft: '-5px' }}>
 								<ul>
 								  {selectedOptionsList.map((selected, index) => (
 									<li key={selected.id} className='selected-option-style' /*style={{ selectedOptionStyle }}*/>
 									  <span style={{ flex: 1 }}>{selected.option}</span>
 									  <button style={countMinusBtnStyle} onClick={() => handleQuantityChange(selected.id, -1)}>━</button>
 									  <span style={{ marginLeft: '10px' }}>{selected.quantity}</span>
 									  <button style={countPlusBtnStyle} onClick={() => handleQuantityChange(selected.id, 1)}>┾</button>
 									  <span style={{ flex: 1, marginLeft: '40px' }}>{(selected.totalPrice).toLocaleString()}원</span>
					...
					<div style={{ marginTop: '90px', display: 'flex', flexDirection: 'column', alignItems: 'center', }}>
						{!hideImg && (
							...
							imageList.slice(0,1).map((imageInfo, index) => (
								<img className='detail-img-reduce-style' src={imageInfo} alt={"불러오기 실패1"} />
							))
  							) : (
						...
						{/*<h2 style={{marginTop: '50px'}}>Review</h2>*/}
						
						<h1 className='avg-grade' /*style={{marginBottom:'50px', marginLeft:'0px',}}*/>★  {avgGrade}/5</h1>
			  ...
			  {productData.map((reviewInfo, index) => (
				<div className='review-window' /*style={{display: 'flex', flexDirection: 'column', position: 'relative', left:'100px',}}*/>
				  ...
				  <div className='option-text' /*style={{position: 'relative', left:'120px', top:'-30px',}}*/>
					...
					<span className='review-msg' /*style={{position: 'relative', left:'120px', top:'-40px',whiteSpace: 'pre-line', maxWidth: '60ch',}}*/>{reviewInfo.ReviewMsg}</span>
