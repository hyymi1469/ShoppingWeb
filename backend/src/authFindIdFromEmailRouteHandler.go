...
func AuthFindIdFromEmailRouteHandler(c *fiber.Ctx) error {

	var reqAuthFindIdFromEmail ReqAuthFindIdFromEmail
	if err := c.BodyParser(&reqAuthFindIdFromEmail); err != nil {
		Logger.Error(err)
		return c.Status(Error_Parse).JSON(fiber.Map{"error": ""})
	}

	reqEmail := reqAuthFindIdFromEmail.Email

	// 이미 가입된 이메일인지 확인
	if IsJoinedEmail(reqEmail) == false {
		Logger.Error("Not joined. reqEmail:", reqEmail)
