...
func AuthFromEmailRouteHandler(c *fiber.Ctx) error {

	var reqAuthFromEmail ReqAuthFromEmail
	if err := c.BodyParser(&reqAuthFromEmail); err != nil {
		Logger.Error(err)
		return c.Status(Error_Parse).JSON(fiber.Map{"error": ""})
	}

	reqEmail := reqAuthFromEmail.Email

	// 이미 가입된 이메일인지 확인
	if IsJoinedEmail(reqEmail) == true {
		Logger.Error("Already Joined. mail:", reqEmail)
		return c.Status(Error_AlreadyAuthedMail).JSON(fiber.Map{"error": ""})
	}
